        ...
        while idx < numsLen:
            ...
            while jdx < numsLen:
                ...
                if target in seenNums:
                    ...
                    jdx += 1
                    # if we find a solution using a specific jdx, we dont want to duplicate that solution so we increment it until we reach a new number
                    # we dont want to do this during the "solution not found" path because we will skip over solutions that may use duplicate but non-repeated (different indeces) numbers
